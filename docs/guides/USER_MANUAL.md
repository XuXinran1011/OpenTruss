...
**角度吸附功能**：
- 在 Trace Mode 下拖拽构件时，系统会自动将角度吸附到标准角度（45°、90°、180°）
- 如果按住 `Alt` 键，可以临时禁用角度吸附，允许自定义角度
- 角度被吸附后，会显示提示信息："角度已吸附到 90°"
...
3. 点击右侧面板的"提交审批"按钮
4. 系统会自动执行以下验证：
   - **构造校验**：检查角度是否符合标准、Z轴参数是否完整
   - **拓扑校验**：检查是否有悬空端点、孤立元素
   - **数据完整性**：检查是否有构件缺少高度、材质或几何数据
5. 验证通过后，检验批状态变为 "SUBMITTED"
6. 如果验证失败，系统会显示错误信息，要求修复后重新提交

**常见问题**:
- **验证失败**: 检查是否有构件缺少高度、材质、拓扑不闭合或存在悬空端点/孤立元素
- **角度不符合标准**: 在 Trace Mode 中重新调整构件角度，系统会自动吸附到标准角度（45°、90°、180°）
- **Z轴参数缺失**: 在 Lift Mode 中为 Wall 和 Column 类型的构件设置 height 和 base_offset
- **状态未更新**: 刷新页面或重新选择检验批
...
1. 在右侧面板切换到"规则管理"标签页
2. 系统显示所有可用的检验批划分规则：
   - **按楼层划分 (BY_LEVEL)**: 根据构件的楼层（Level）自动分组创建检验批
   - **按区域划分 (BY_ZONE)**: 根据构件的区域（Zone）自动分组创建检验批
   - **按楼层+区域划分 (BY_LEVEL_AND_ZONE)**: 根据楼层和区域的组合自动分组创建检验批

#### 预览规则效果

1. 在"规则管理"面板中选择一个规则
2. 输入要预览的分项 ID（Item ID）
3. 点击"预览规则效果"按钮
4. 系统显示预览信息：
   - 预计创建的检验批数量
   - 未分配构件总数
...
3. 系统自动执行拓扑校验并显示结果：
   - **校验状态**: 显示"校验通过"或"校验失败"
   - **悬空端点**: 列出所有连接数小于2的元素（悬空端点）
   - **孤立元素**: 列出所有没有任何连接的元素（孤立元素）

#### 处理校验问题

1. **高亮问题元素**: 点击悬空端点或孤立元素旁边的"高亮"按钮，系统会在画布中高亮显示该元素
2. **修复问题**: 
   - 悬空端点：检查元素是否正确连接到其他元素，在 Trace Mode 中修复连接
   - 孤立元素：检查元素是否正确归类到检验批，或需要连接到其他元素
3. **重新校验**: 修复问题后，点击"重新校验"按钮更新校验结果

**注意**: 
- 拓扑校验在检验批状态转换为 SUBMITTED 时自动执行
- 如果校验失败，系统会阻止状态转换，要求修复问题后重新提交
